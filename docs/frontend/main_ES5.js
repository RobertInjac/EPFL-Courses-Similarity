// This file contains the production, retro-compiled ES-2015 Javascript code
// The original has been retro-compiled for ES-2015 to fix compatibility issues
// The original code can be found in frontend/main.js

"use strict";

var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }();

var NODE_COLOR = "rgba(145, 145, 145, 1)";
var EDGE_COLOR = "rgba(145, 145, 145, 0.8)";
var SELECTED_EDGE_COLOR = "rgba(0, 0, 0, 0.8)";
var BACKGROUND_NODE_COLOR = "rgba(142,136,130, 0.1)";
var BACKGROUND_EDGE_COLOR = "rgb(229,229,229)";

var HIGHLIGHT_DEPTH = 2;
var BATCH_SIZE = 50;
var SHOW_LABELS_THRESHOLD = 0.5;
var NODE_SIZE = 13;
var current_node_size = NODE_SIZE;
var MAX_NODE_SIZE = 75;
var FONT_SIZE = 12;
var ZOOM_CONSTRAIN = [0.05, 2.5];
var HIDE_ARROWHEADS_THRESHOLD = 0.25;
//const HIGHLIGHT_COLORS = ['#5C5C5C', '#454545', '#2E2E2E', '#171717', '#000000'];
var HIGHLIGHT_COLORS = ['rgb(180, 180, 180)', 'rgb(100, 100, 100)', 'rgb(0, 0, 0)'];
...
var course_sections = {};
var adjacency_list = {};
var drawing_timer = void 0;
var drawing_complete = true;
var show_labels = true;
var arrowheads_visible = true;
var ZOOMED = false;
...
var width = document.getElementsByClassName('svg-container')[0].clientWidth;
var height = document.getElementsByClassName('svg-container')[0].clientHeight;

// create the SVG element that will contain the nodes and edges
var svg = d3.select("div#graph").append("svg").attr("width", "100%").attr("height", "100%").classed("svg-content", true);
var g = svg.append("g");

// triangle at the end of edges (arrowheads)
svg.append('defs').append('marker').attrs({ 'id': 'arrowhead',
    'viewBox': '-0 -5 10 10',
    'refX': 29,
    'refY': 0,
    'orient': 'auto',
    ...
    'xoverflow': 'visible' }).append('svg:path').attr('d', 'M 0,-5 L 10 ,0 L 0,5').attr('fill', EDGE_COLOR).style('stroke', 'none');

// handle zoom events on the svg
var zoom_handler = d3.zoom().scaleExtent(ZOOM_CONSTRAIN).on("start", function () {
    return zoomBegin();
}).on("end", function () {
...
function import_data(path, display_callback) {
    ...
    graph.edges.forEach(function (e) {
        edges['eid_' + e.id] = {
            ...
            'x1': graph.nodes.filter(function (node) {
                return node.id == e.source;
            })[0].viz.position.x,
            'y1': graph.nodes.filter(function (node) {
                return node.id == e.source;
            })[0].viz.position.y,
            'x2': graph.nodes.filter(function (node) {
                return node.id == e.target;
            })[0].viz.position.x,
            'y2': graph.nodes.filter(function (node) {
                return node.id == e.target;
            })[0].viz.position.y,
            'id': 'eid_' + e.id,
            'type': e.label
    ...
    Object.keys(edges).map(function(key){return edges[key]}).forEach(function (e) {
...
function display_graph() {
    ...
    // draw the edges
    g.selectAll(".link").data(Object.keys(edges).map(function(key){return edges[key];})).enter().append("line").attr("x1", function (d) {
        return d.x1;
    }).attr("y1", function (d) {
        return d.y1;
    }).attr("x2", function (d) {
    ...
    }).style('stroke', EDGE_COLOR).attr('marker-end', 'url(#arrowhead)');

    // draw the nodes
    g.selectAll(".node").data(Object.keys(nodes).map(function(key){return nodes[key];})).enter().append("circle").attr("cx", function (d) {
        return d.x;
    }).attr("cy", function (d) {
        return d.y;
    }).attr("class", "node").attr("id", function (d) {
        return d.id;
    }).style('fill', NODE_COLOR).on("click", function (d) {
        on_click_node(d.id);
    });

    // add text above nodes
    g.selectAll("text").data(Object.keys(nodes).map(function(key){return nodes[key];}), function (d) {
        return d.id;
        ...
        return 'title_' + d.id;
    }).attr('class', 'node_title').text(function (d) {
        return d.title;
    });
}
...
function import_descriptions() {
    d3.tsv("./frontend/data/descriptions.csv").row(function (row) {
...
function import_sections() {
    d3.tsv("./frontend/data/sections.csv").row(function (row) {
...
import_data('./frontend/data/graph.gexf', function () {
    display_graph();
});
//import_descriptions();
//import_sections();
...
function node_is_visible(node, width, height, transf) {
    ...
    var _get_new_coordinates = get_new_coordinates(node.x, node.y, transf);

    var _get_new_coordinates2 = _slicedToArray(_get_new_coordinates, 2);

    x = _get_new_coordinates2[0];
    y = _get_new_coordinates2[1];
...
function edge_is_visible(edge, width, height, transf) {
    ...
    var _get_new_coordinates3 = get_new_coordinates(edge.x1, edge.y1, transf);

    var _get_new_coordinates4 = _slicedToArray(_get_new_coordinates3, 2);

    x1 = _get_new_coordinates4[0];
    y1 = _get_new_coordinates4[1];

    var _get_new_coordinates5 = get_new_coordinates(edge.x2, edge.y2, transf);

    var _get_new_coordinates6 = _slicedToArray(_get_new_coordinates5, 2);

    x2 = _get_new_coordinates6[0];
    y2 = _get_new_coordinates6[1];

    var new_edge = [x1, y1, x2, y2];
    var left_border = [0, 0, 0, height];
    var top_border = [0, 0, width, 0];
    var right_border = [width, 0, width, height];
    var bottom_border = [0, height, width, height];
...
function highlight_selection(node_id) {
    var is_source = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    var distance = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : HIGHLIGHT_DEPTH;
    var already_seen = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
    ...
    if (distance > 0) {
        ...
        var adjacent_edges = adjacency_list[node_id].map(function (edge_id) {
        ...
        var unvisited_nodes = adjacent_edges.map(function (edge) {
            return edge.target;
        }).filter(function (nid) {
            return !already_seen.includes(nid);
        });
        unvisited_nodes.forEach(function (nid) {
            return already_seen.push(nid);
        });
        unvisited_nodes.forEach(function (nid) {
            return highlight_selection(nid, false, distance - 1, already_seen);
        });
    }
}
...
function highlight_edge(edge, distance_to_leaf) {
    var selected_edge = d3.select('#' + edge.id);
...
function clear_selection() {
    var new_selection = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
...
function zoomed(width, height, transform) {
    ...
    // update node size
    var radius = get_node_size(transform.k);
    current_node_size = radius;
    d3.selectAll('.node').filter(function (node) {
        return node_is_visible(node, width, height, transform);
    }).style("r", radius);
...
function draw_edge_batch(batch, line_width) {
    g.selectAll('.link').data(batch, function (e) {
        return e.id;
    })
    //.style('stroke-width', 0)
    //.transition().duration(200)
    .style("stroke-width", line_width).style('display', 'block');
}
...
function redraw_edges(width, height, transform) {
    ...
    var line_width = get_line_width(transform.k);
    var batch_number = 0;
    var visible_edges = d3.selectAll('.link').filter(function (edge) {
        return edge_is_visible(edge, width, height, transform);
    }).data();
    // if there is a selection, only draw selected edges
    if (EDGE_SELECTION.length > 0) visible_edges = visible_edges.filter(function (edge) {
        return EDGE_SELECTION.includes(edge);
    });
    ...
    drawing_timer = d3.timer(function () {
        var batch = visible_edges.splice(0, Math.min(BATCH_SIZE, visible_edges.length));
...
function translate_to(nid, scale_factor) {
    ...
    var _get_new_coordinates7 = get_new_coordinates(node.x, node.y, d3.zoomTransform(svg.node())),
        _get_new_coordinates8 = _slicedToArray(_get_new_coordinates7, 2),
        x = _get_new_coordinates8[0],
        y = _get_new_coordinates8[1];

    return d3.zoomIdentity.translate(width / 2, (height - parseInt(d3.select('#infopanel').style('height'))) / 2).scale(scale_factor).translate(-node.x, -node.y);
}

// translates the graph to the center of the screen
function translate_to_center(scale_factor) {
    return d3.zoomIdentity.translate(width / 2, height / 2).scale(scale_factor);
}
...
function show_node_labels(transform) {
    ...
    if (transform.k > SHOW_LABELS_THRESHOLD) {
        var font_size = get_label_font_size(transform.k);

        // filter out the nodes that do not appear on the screen
        var visible_nodes = d3.selectAll('.node').filter(function (node) {
            return node_is_visible(node, width, height, transform);
        }).data();

        // if there is a selection, only show labels of selected nodes
        if (NODE_SELECTION.length > 0) visible_nodes = visible_nodes.filter(function (node) {
            return NODE_SELECTION.includes(node.id);
        });

        // display the label for each visible node
        d3.selectAll('.node_title').data(visible_nodes, function (d) {
            return d.id;
        }).style('display', 'block').style('font-size', font_size);
    }
}
...
function fill_infopanel(nid) {
    ...
    // title
    infopanel.append('div').append('strong').text(nodes[nid].title);
    // description
    infopanel.append('div').text(course_descriptions[nid]);
    infopanel.append('br');
    // sections
    if (course_sections[nid]) {
        var text = '<strong style="">This course is thaught to: </strong> <br/>';
    ...
    if (adjacency_list[nid]) {
        ...
        var related_courses = adjacency_list[nid].map(function (eid) {
            return edges[eid].target;
        });
        related_courses.forEach(function (nid) {
            infopanel.append('span').text(nodes[nid].title).attr('class', 'related_course').on('click', function () {
                on_click_node(nid);
                g.select('circle#' + nid).style('stroke-width', '0px');
            }).on('mouseover', function () {
                d3.select(this).style('border-color', 'rgba(100, 100, 100, 1)').style('cursor', 'pointer');
                g.select('circle#' + nid).style('stroke', 'black').style('stroke-width', current_node_size / 6 + 'px');
            }).on('mouseout', function () {
                d3.select(this).style('border-color', 'transparent').style('cursor', 'default');
                g.select('circle#' + nid).style('stroke-width', '0px');
            });
        });
    }
...
function search(query) {
    ...
    var result_nids = Object.keys(nodes).map(function(key){return nodes[key];}).filter(function (node) {
        return String(node.title).toLowerCase().includes(query.toLowerCase());
    });
    // keep only the first 10
    result_nids = result_nids.slice(0, 10);

    var cells = d3.select('#search_results_container').selectAll('.search_results_cell').data(result_nids, function (d) {
        return d.id;
    });
